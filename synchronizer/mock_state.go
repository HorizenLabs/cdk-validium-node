...
// AddAttestationId provides a mock function with given fields: ctx, attestationId, dbTx
func (_m *stateMock) AddAttestationId(ctx context.Context, attestationId *state.AttestationId, dbTx pgx.Tx) error {
	ret := _m.Called(ctx, attestationId, dbTx)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *state.AttestationId, pgx.Tx) error); ok {
